package com.agileboot.admin.modular.sys.system.role.model;
...
import com.agileboot.admin.modular.sys.system.role.db.SysRoleMenuEntity;
import com.agileboot.admin.modular.sys.system.role.db.SysRoleMenuService;
...
public class RoleModel extends SysRoleEntity {
    ...
    public void checkRoleNameUnique() {
        if (roleService.isRoleNameDuplicated(getRoleId(), getRoleName())) {
            throw new ApiException(Business.ROLE_NAME_IS_NOT_UNIQUE, getRoleName());
        }
    }
    ...
    public void checkRoleKeyUnique() {
        if (roleService.isRoleKeyDuplicated(getRoleId(), getRoleKey())) {
            throw new ApiException(Business.ROLE_KEY_IS_NOT_UNIQUE, getRoleKey());
        }
    }
    ...
    public void generateDeptIdSet() {
        ...
        if (deptIds.size() > new HashSet<>(deptIds).size()) {
            throw new ApiException(Business.ROLE_DATA_SCOPE_DUPLICATED_DEPT);
        }
